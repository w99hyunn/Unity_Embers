...
namespace NOLDA
{
    public class MapDirector : MonoBehaviour
    {
        ...
        private async Awaitable LoadInGameCoroutine()
        {
            ...
            await UpdateChunks(Director.Game.playerData.Position);
        ...
        public async Awaitable UpdateChunks(Vector3 playerPosition)
        {
            ...
            for (int x = -(Director.Game.LoadRange); x <= Director.Game.LoadRange; x++)
            {
                for (int z = -(Director.Game.LoadRange); z <= Director.Game.LoadRange; z++)
                {
        ...
        {
            int x = Mathf.FloorToInt(position.x / Director.Game.ChunkSize);
            int z = Mathf.FloorToInt(position.z / Director.Game.ChunkSize);
